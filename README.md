...
> 把「资产发现 → 服务识别 → 指纹匹配 → 漏洞验证 → 报告输出」这条渗透流水线，
> 做成可插拔、可观测、可复现的异步平台。
...
**验证状态**：`Dockerfile` 与 `docker-compose.yml` 已完成，并通过 `docker compose config`
语法校验（含 `ASP_API_TOKEN` 强制检查的行为验证）。但开发机上的 Docker 守护进程无法启动
（`wsl.exe` 被安全策略拦截），因此**镜像未经实际构建验证**。首次使用时如遇到问题，
欢迎提 issue。
...
基线探测要求 ≥2 次命中而不是 1 次。单次命中可能是运营商 DNS 劫持（国内校园网/运营商把不存在的域名劫持到广告页）。
要求两个独立随机域名命中同一 IP，才能排除这种偶发噪声。

**已知局限**：只检测顶层泛解析；CDN 场景下泛解析与真实记录可能指向同一 CDN IP，会误杀。
...
报告里按置信度排序，人工复核可以从最可信的开始。置信度本身也保留了「有多确定」
这一维信息，比只输出布尔值更有用。
...
这是安全工具的现实约束：被部署到客户环境时，少一个依赖就少一次「装不上」的现场事故。
