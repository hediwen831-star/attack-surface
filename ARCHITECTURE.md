...
本文说明技术选型（用了什么、为什么用它）与核心链路的实现方式，
并记录各处的设计取舍及其理由。
...
表格对齐说明：`tabulate` 这类库按字符数计算宽度，但中文在终端里占 2 列而 ASCII
占 1 列，导致中文表格永远对不齐。自研的 `_pad()` 按 `ord(ch) > 0x2E80` 判断宽字符，
实际效果见 README 的截图。
...
| 工具 | 用途 | 配置位置 |
|---|---|---|
| **pytest** ≥ 8.0 | 单元测试（355 个用例） | `pyproject.toml` |
...
检测引擎里最容易埋雷的地方是「让用户写表达式」。如果用 `eval()` 执行 YAML 里的条件，
加载的就不是数据，而是**任意代码** —— 任何一条从社区下载的 PoC 都能在目标机器上执行命令。
安全工具自身存在 RCE 是最严重的一类缺陷。
...
这条边界不经过任何能构造出任意 Python 表达式的路径，由测试
`test_dsl_rejects_non_whitelisted_syntax` 守着：
...
程序用默认值 100 跑了一小时，谁也不知道为什么没生效。拼错的配置应当立即报错，
而不是静默降级。
...
阈值取 ≥2 而不是 ≥1 的原因：单个随机域名偶然解析成功，可能是运营商 DNS 劫持 ——
国内校园网/运营商经常把不存在的域名劫持到广告页。要求两个**独立的**随机域名命中
**同一个** IP，才能排除这种偶发噪声。

判定用 `issubset` 而不是 `isdisjoint`：
...
取舍取向是宁可多留一条假资产让人工复核，也不漏掉一条真实资产。
误报可以筛掉，漏报没法补救。
...
### 4.3 可达性判定（比命中数更基础的维度）

负向对照解决的是「**误报**」—— 把不存在的漏洞报成存在。

但还有一类问题在它对面，而且更危险：**「什么都没扫到」被报告成「没扫到漏洞」**。

| 情况 | hit_count | 使用者看到的 | 真实含义 |
|---|---|---|---|
| 目标确实没有该漏洞 | 0 | 未发现漏洞 | 结论成立 |
| 目标连不上 | 0 | 未发现漏洞 | 结论不成立，一个请求都没打出去 |
| 代理返回 502 | 0 | 未发现漏洞 | 结论不成立，同上，而且更隐蔽 |

这三种情况在只看 `hit_count` 时输出完全一样。所以引入一个独立维度：

```python
responses_ok               # 成功读到的响应数
responses_server_error     # 其中 5xx 的数量
target_reachable           # responses_ok - responses_server_error > 0
```

**为什么必须把 5xx 单独扣掉**：实测踩到的坑 ——
本机跑着 HTTP 代理时，向已关闭的端口发请求**不会**得到「连接被拒」，
而是代理返回 **502 Bad Gateway**。502 是一个结构完整的 HTTP 响应，
它能通过 `resp.ok` 检查，也能被计入"拿到了响应"。

所以判据收紧成「至少有一个**非 5xx** 的响应」：

- 5xx 无法说明被测应用处于正常工作状态，服务可能根本没起来
- 404 反而是有效信号：说明应用确实在处理路由，只是这个路径不存在
- 只要有一个非 5xx 响应就判可达（个别路径 500 在扫描中很常见，
  不能因此否定整次扫描）

**不可达时的处置**：CLI 打印显式警告（而不是安静地输出「未发现漏洞」），
并让退出码变成 `2`：
...
分开退出码的意义在 CI 里最明显：跑基准测试时，
「靶场没起来」和「靶场真的没洞」需要完全不同的处置方式。

这一节的设计取向是：宁可把「不知道」报成错误，也不要把它报成「没问题」。
...
**为什么 `or` 条件下单个匹配器命中时上限是 0.8**：单一特征（尤其单个关键词）
的误报率显著高于多特征组合。本项目的 Swagger PoC 就是实例：只匹配关键词
`swagger` 时误报了 3 次（因为 404 页面回显了请求路径）。
...
运行时不需要它；缺失时测试自动跳过而不是失败。自行实现的部分不能自行验证，
需要独立参照实现。
...
这个模块的能力是「触发主动扫描」，所以**它本身就是一个攻击面**。

三道约束：

**① 默认只绑定 `127.0.0.1`**

**② 绑定非回环地址时强制要求 token** —— 没设置 `ASP_API_TOKEN` 直接拒绝启动

为什么是「拒绝启动」而不是「打印警告」：**警告没人看**。
一个无认证的扫描接口暴露在公网上，等于把工具变成别人手里的攻击跳板，
而且流量从你的 IP 出去。

**③ 触发扫描的端点全部要求 `X-API-Token` 头**（只读端点不强制 —— 看报告不产生流量）

token 比较用 `hmac.compare_digest` 而不是 `==` ——
后者会因为提前返回而泄漏时序信息，理论上可被用来逐字节爆破 token。

**为什么 FastAPI 是可选依赖**

核心能力只需要 3 个运行时依赖。把 FastAPI + uvicorn 做成 `[api]` extra，
是为了让「只想用命令行」的人不必为此装一整套 Web 框架。
没装时 `asp serve` 会给出明确的安装提示，而不是抛一个 ImportError 堆栈。
...
没有 API key 时用启发式规则兜底。一个「需要联网 + 需要付费 key 才能跑」的功能，
在别人 clone 你的项目时等于不存在 —— 而这恰恰是最需要被看到的部分。

**③ 必须能验证效果**

「引入了 LLM 降噪」不构成结论，「误报率从 X% 降到 Y%」才是。
所以内置标注样本 + 评估函数，可以量化：

```
$ asp triage --self-test
准确率      83.3%
有效覆盖率  83.3%
误报召回率  100.0%     标注样本里的误报全部被抓出
真阳性识别  2/3        剩下一条报「不确定」，没有猜测
```

**不确定计为错误**（保守计算）—— 否则「什么都不判」会得到满分。

启发式里没有收录「`key = value` 形式的配置项」这条看似通用的规则。
因为它太宽松：SQL 语句、报错信息里到处都是 `id = 1`，
加上它之后错误页会被判成真阳性。规则写得太宽松会导致误判，
而误判比不判更糟，它会让人放松警惕。

这与 PoC 引擎、指纹识别的置信度设计是同一个原则在三个地方的重复。
...
| 单元测试 | 355 个用例，约 3,800 行 |
| 行覆盖率 | 74%（`asp/cli.py` 52% / 引擎 86% / 匹配器 92%） |
...
入口层曾经是整个项目唯一 0% 覆盖的模块（433 行），当时的判断是
「CLI 只是把各模块串起来，逻辑简单」。补上覆盖后，第一条新测试就抓出一个
从项目建立起就存在的 bug：

```
asp poc run <target> --json        # 终端无输出
asp poc run <target> --json -o x   # 正常
```

`cmd_poc_run` 的 JSON 分支把 `to_json(result)` 赋给 `text` 后没有打印，
`print(text)` 只在 `-o` 那条路径上生效。其他五个子命令的 `--json`
都调用了 `print(json.dumps(...))`，只有这一个漏了。

该缺陷只在特定参数组合下出现，而非某条代码路径上：手工测试时通常会顺手加
`-o` 确认文件已写出，那条有问题的组合因此走不到。后果是静默的 ——
`jq` 拿到空输入、`$(...)` 得到空字符串，看起来像没扫到东西，
而不像工具故障。

入口层有一类只有它才会出现的缺陷：退出码语义、输出格式契约、参数透传。
现在 `tests/test_cli.py` 里有 31 条断言守着，其中两条是端到端用例 ——
只在 HTTP 客户端这一层造假，让真实的 `scan_target` 跑完整条链路。

**为什么测试全部离线可跑**：测绘工具天生依赖外部服务（crt.sh、DNS、目标站点），
但测试不能依赖 —— 否则 CI 会因为某个第三方站点抖动而变红，
红过几次之后测试就会被忽略。
...
本节措辞刻意保守：本项目不声称任何「首创」。
攻击面测绘、子域名枚举、YAML PoC 引擎都已有成熟方案，说明差异比夸大更有意义。
...
本项目是「把扫描器内部机制完整实现出来」的教学型平台，而非另一个功能更全的扫描器。

功能覆盖度上，本项目远不如 amass 或 nuclei。它的价值在另外三个地方：

**① 每一层都有可追溯的实现依据**

用 amass 只能回答「我用 amass 做了资产发现」。用本项目可以回答
「泛解析怎么判、为什么用 `issubset` 而不是 `isdisjoint`、负向对照怎么设计、
为什么 DSL 不能用 `eval`」。
