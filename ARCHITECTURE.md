...
>
> 关于「为什么不用 XX」「这里为什么这么设计」的完整论证，都在这里。
...
这三种情况在只看 `hit_count` 时**输出完全一样**。所以引入一个独立维度：
...
- 5xx 证明不了「我们扫的是一个正常工作的应用」—— 服务可能根本没起来
- 而 **404 反而是好信号** —— 说明应用确实在处理路由，只是这个路径不存在
...
启发式里没有「`key = value` 形式的配置项」这条看似通用的规则。
...
**关于 `cli.py` 那块短板（一个值得记下来的教训）**

入口层曾经是整个项目**唯一 0% 覆盖**的模块（433 行）。
当时没测它的理由是："CLI 只是把各模块串起来，逻辑简单。"

补上之后，第一条新测试就抓出一个从项目建立起就存在的 bug：

```
asp poc run <target> --json        # ← 终端一片空白，什么都不打印
asp poc run <target> --json -o x   # ← 正常
```

`cmd_poc_run` 的 JSON 分支把 `to_json(result)` 赋给 `text` 就没了下文 ——
`print(text)` 只在 `-o` 那条路径上间接生效。其他五个子命令的 `--json`
都老老实实 `print(json.dumps(...))`，**只有这一个漏了**。

这个 bug 的形态很典型：**它藏在"参数组合"里而不是"代码路径"里**。
手工测试时你总会顺手加个 `-o` 看看文件写没写出来，
于是那条有问题的组合永远走不到。而它的后果是静默的 ——
`jq` 拿到空输入、`$(...)` 得到空字符串，
看起来像"没扫到东西"，不像"工具坏了"。

**结论没有任何一层是"简单到不用测"的。** 入口层藏着整整一类
只有它才会出现的缺陷：退出码语义、输出格式契约、参数透传。
现在 `tests/test_cli.py` 里有 29 条断言守着，其中两条是端到端用例 ——
